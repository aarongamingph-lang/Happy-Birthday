(function(window){
    ...
    Seed.prototype = {
        ...
        drawHeart: function() {
            ...
            var radius = 18 * scale;
            var glow = ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, radius * 2.8);
            glow.addColorStop(0, "rgba(255, 255, 255, 1)");
            glow.addColorStop(0.18, "rgba(255, 185, 220, 0.96)");
            glow.addColorStop(0.45, "rgba(255, 34, 96, 0.72)");
            glow.addColorStop(1, "rgba(255, 34, 96, 0)");
            ctx.save();
            ctx.fillStyle = glow;
            ctx.beginPath();
            ctx.arc(point.x, point.y, radius * 2.8, 0, 2 * Math.PI);
            ctx.closePath();
            ctx.fill();
            ctx.fillStyle = "rgba(255, 236, 250, 0.95)";
            ctx.beginPath();
            ctx.arc(point.x, point.y, Math.max(3, radius * 0.46), 0, 2 * Math.PI);
            ctx.closePath();
            ctx.fill();
            ctx.restore();
        },
        drawCirle: function() {
            ...
            var point = cirle.point, scale = cirle.scale, radius = cirle.radius * scale;
            var glow = ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, radius * 5);
            glow.addColorStop(0, "rgba(255, 255, 255, 1)");
            glow.addColorStop(0.22, "rgba(255, 190, 224, 0.98)");
            glow.addColorStop(0.55, "rgba(255, 0, 72, 0.72)");
            glow.addColorStop(1, "rgba(255, 0, 72, 0)");
            ctx.save();
            ctx.fillStyle = glow;
            ctx.beginPath();
            ...
            ctx.beginPath();
    	    ctx.arc(point.x, point.y, Math.max(3, radius), 0, 2 * Math.PI);
            ctx.closePath();
            ctx.fill();
            ctx.restore();
        },
        ...
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
            var point = cirle.point, scale = cirle.scale, radius = 52;
    ...
    Tree.prototype = {
        ...
        moveTo: function(k, x, y) {
            ...
            var dx = x - point.x;
            var dy = y - point.y;
            var i = Math.abs(dx) <= speed ? x : point.x + (dx > 0 ? speed : -speed);
            var j = Math.abs(dy) <= speed ? y : point.y + (dy > 0 ? speed : -speed);
